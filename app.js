...
function getCategory() {
    fetch('https://69b3c331e224ec066bdd03a6.mockapi.io/api/v1/Category')
        .then(res => res.json())
        .then(categ => {
            categ.map(categ => categoryList.innerHTML += `<li onclick="sortData('${categ.name}')" class="bg-grey-400 p-2 border border-green-400 rounded-xl cursor-pointer text-green-400 text-md">${categ.name}</li>`)
        }
    )
}
